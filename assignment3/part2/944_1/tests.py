...
# Tests based on generated_specifications_944.txt

def test_spec_1_length_correctness():
    """Specification 1: When digits exist, function returns valid index (adapted for single return value)."""
    text = "abc123def"
    result = solution.num_position(text)
    digit_count = sum(1 for i in range(len(text)) if text[i].isdigit())
    # If digits exist, should return a valid index (not -1)
    if digit_count > 0:
        assert result != -1
...
def test_spec_2_index_validity():
    """Specification 2: Returned index is valid and points to a digit character."""
    test_cases = [
        "there are 70 flats in this apartment",
        "every adult have 32 teeth",
        "i463kar0q9w3rtkk7r5im0slj736p7edphcda",
        "abc123def456"
    ]
    for text in test_cases:
        result = solution.num_position(text)
        if result != -1:
            assert 0 <= result < len(text), f"Index {result} out of bounds for text length {len(text)}"
            assert text[result].isdigit(), f"Character at index {result} should be a digit, got '{text[result]}'"

def test_spec_3_order_preservation():
    """Specification 3: Function returns the first (leftmost) digit index (order preservation)."""
    text = "a1b2c3d4"
    result = solution.num_position(text)
    assert result == 1, "Should return index of first digit '1', not later digits"
    
    text = "x5y6z7"
    result = solution.num_position(text)
    assert result == 1, "Should return index of first digit '5'"
    
    text = "012345"
    result = solution.num_position(text)
    assert result == 0, "Should return index of first digit '0'"

def test_spec_4_correct_index():
    """Specification 4: Character at returned index matches the digit at that position."""
    test_cases = [
        ("abc123", 3, '1'),
        ("hello5world", 5, '5'),
        ("9test", 0, '9'),
    ...
    for text, expected_idx, expected_char in test_cases:
        result = solution.num_position(text)
        assert result == expected_idx, f"Expected index {expected_idx}, got {result}"
        assert text[result] == expected_char, f"Expected character '{expected_char}' at index {result}, got '{text[result]}'"

def test_spec_5_no_non_digits():
    """Specification 5: Returned index points to a digit, not a non-digit character."""
    text = "abc!@#123def"
    result = solution.num_position(text)
    assert result == 6, "Should skip non-digits and return index of first digit"
    assert text[result].isdigit(), "Character at returned index must be a digit"
    
    text = "!@#$%^&*()"
    result = solution.num_position(text)
    assert result == -1, "No digits in string, should return -1"

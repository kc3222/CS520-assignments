...
# Tests based on generated_specifications_491.txt

def test_sum_gp_r_equals_one():
    """Test sum_gp when r == 1: sum must be exactly a * n."""
    assert solution.sum_gp(5, 3, 1) == 15.0  # 5 * 3 = 15
    assert solution.sum_gp(2, 10, 1) == 20.0  # 2 * 10 = 20
    assert solution.sum_gp(7, 4, 1) == 28.0   # 7 * 4 = 28
    assert solution.sum_gp(1, 1, 1) == 1.0    # 1 * 1 = 1

def test_sum_gp_r_not_equals_one():
    """Test sum_gp when r != 1: sum must use closed-form formula a * (1 - r**n) / (1 - r)."""
    # Test with r = 2, a = 1, n = 5: 1 * (1 - 2**5) / (1 - 2) = 1 * (1 - 32) / (-1) = 31
    assert solution.sum_gp(1, 5, 2) == 31.0
    # Test with r = 3, a = 2, n = 4: 2 * (1 - 3**4) / (1 - 3) = 2 * (1 - 81) / (-2) = 80
    assert solution.sum_gp(2, 4, 3) == 80.0
    # Test with r = 0.5, a = 4, n = 3: 4 * (1 - 0.5**3) / (1 - 0.5) = 4 * (1 - 0.125) / 0.5 = 7
    assert solution.sum_gp(4, 3, 0.5) == 7.0

def test_sum_gp_n_equals_one():
    """Test sum_gp when n == 1: sum must always equal the first term a."""
    assert solution.sum_gp(5, 1, 2) == 5.0
    assert solution.sum_gp(3, 1, 10) == 3.0
    assert solution.sum_gp(7, 1, 0.5) == 7.0
    assert solution.sum_gp(0, 1, 5) == 0.0

def test_sum_gp_a_equals_zero():
    """Test sum_gp when a == 0: entire geometric progression sums to zero."""
    assert solution.sum_gp(0, 5, 2) == 0.0
    assert solution.sum_gp(0, 10, 3) == 0.0
    assert solution.sum_gp(0, 1, 7) == 0.0
    assert solution.sum_gp(0, 100, 0.5) == 0.0

def test_sum_gp_r_equals_zero():
    """Test sum_gp when r == 0: sum is always just the first term a."""
    assert solution.sum_gp(5, 3, 0) == 5.0
    assert solution.sum_gp(7, 10, 0) == 7.0
    assert solution.sum_gp(2, 1, 0) == 2.0
    assert solution.sum_gp(0, 5, 0) == 0.0
